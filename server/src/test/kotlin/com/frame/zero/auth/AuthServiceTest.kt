...
class AuthServiceTest {
  ...
  @Test
  fun `register normalizes email to lowercase and trims whitespace`() = runTest {
  ...
  @Test
  fun `register persists hashed password and creates a refresh token row`() = runTest {
    val users = FakeUserRepository()
  ...
  @Test
  fun `register stores refresh token hashed not in plaintext`() = runTest {
    val tokens = FakeRefreshTokenRepository()
  ...
  @Test
  fun `login with unknown email returns InvalidCredentials`() = runTest {
    val service = makeService()
  ...
  @Test
  fun `refresh with unknown token returns InvalidRefreshToken`() = runTest {
    val service = makeService()
  ...
  @Test
  fun `refresh with valid token revokes the old one and returns a new pair`() = runTest {
    val tokens = FakeRefreshTokenRepository()
    val tokenHasher = TokenHasher()
    val service = makeService(tokens = tokens, tokenHasher = tokenHasher)
    val auth = service.register("u@x.com", "password123", "", "")
  ...
  @Test
  fun `logout with unknown token does not throw`() = runTest {
    val service = makeService()
  ...
  @Test
  fun `me returns null for an unknown user id`() = runTest {
    val service = makeService()
