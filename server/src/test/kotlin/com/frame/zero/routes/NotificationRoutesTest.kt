...
class NotificationRoutesTest {
  ...
  @Test
  fun `GET notifications returns 200 with empty list`() = testApplication {
    val env = TestAppEnv()
  ...
  @Test
  fun `GET notifications without token returns 401`() = testApplication {
    val env = TestAppEnv()
    application { env.configure(this) }
  ...
  @Test
  fun `GET notifications returns unread count`() = testApplication {
    val env = TestAppEnv()
    application { env.configure(this) }
    val userId = UUID.randomUUID()
    val token = env.tokenFor(userId)
  ...
  @Test
  fun `POST notifications read marks all read returns 204`() = testApplication {
    val env = TestAppEnv()
    application { env.configure(this) }
    val userId = UUID.randomUUID()
    val token = env.tokenFor(userId)
  ...
  @Test
  fun `POST notifications read without token returns 401`() = testApplication {
    val env = TestAppEnv()
    application { env.configure(this) }
  ...
  @Test
  fun `POST notifications read with empty ids and all false returns 400`() = testApplication {
    val env = TestAppEnv()
    application { env.configure(this) }
    val userId = UUID.randomUUID()
    val token = env.tokenFor(userId)
