...
class ScheduleRoutesTest {
  ...
  @Test
  fun `GET schedule day view returns 200`() =
    testApplication {
      val env = TestAppEnv()
  ...
  @Test
  fun `GET schedule without token returns 401`() =
    testApplication {
      val env = TestAppEnv()
      application { env.configure(this) }
  ...
  @Test
  fun `GET schedule missing view returns 400`() =
    testApplication {
      val env = TestAppEnv()
      application { env.configure(this) }
      val userId = UUID.randomUUID()
  ...
  @Test
  fun `POST schedule creates event and returns 201`() =
    testApplication {
      val env = TestAppEnv()
      application { env.configure(this) }
      val userId = UUID.randomUUID()
  ...
  @Test
  fun `POST schedule for non-member production returns 403`() =
    testApplication {
      val env = TestAppEnv()
      application { env.configure(this) }
