...
sealed class AppError(val status: HttpStatusCode, val code: String, val humanMessage: String) {
  ...
  data class Conflict(val detail: String) : AppError(HttpStatusCode.Conflict, "CONFLICT", detail)
  ...
  data class ValidationError(val fields: Map<String, String>) :
    AppError(HttpStatusCode.BadRequest, "VALIDATION_ERROR", "Validation failed")

  data class Internal(val detail: String) :
    AppError(HttpStatusCode.InternalServerError, "INTERNAL", detail)
}

class AppException(val error: AppError) : RuntimeException(error.humanMessage)
