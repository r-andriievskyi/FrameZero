...
class TaskRepositoryExposed : TaskRepository {
  ...
  override suspend fun findById(id: UUID): TaskRecord? = dbQuery {
  ...
  override suspend fun findForUser(
    ...
    limit: Int,
    cursor: String?,
  ): Pair<List<TaskRecord>, String?> = dbQuery {
    val rows =
      (TasksTable innerJoin ProductionsTable)
        .selectAll()
        .where {
  ...
  override suspend fun findForUserLimit(userId: UUID, limit: Int): List<TaskRecord> = dbQuery {
    (TasksTable innerJoin ProductionsTable)
      .selectAll()
      .where {
  ...
  override suspend fun countOpenForUser(userId: UUID): Int = dbQuery {
