...
interface NotificationRepository {
  ...
  suspend fun markRead(userId: UUID, ids: List<UUID>)

  suspend fun markAllRead(userId: UUID)

  suspend fun create(userId: UUID, title: String, body: String?): NotificationRecord
}
...
class NotificationRepositoryExposed : NotificationRepository {
  override suspend fun findForUser(
    ...
    limit: Int,
    cursor: String?,
  ): Pair<List<NotificationRecord>, String?> = dbQuery {
    val rows =
  ...
  override suspend fun countUnread(userId: UUID): Int = dbQuery {
    NotificationsTable.selectAll()
  ...
  override suspend fun markRead(userId: UUID, ids: List<UUID>): Unit = dbQuery {
  ...
  override suspend fun markAllRead(userId: UUID): Unit = dbQuery {
    val now = Instant.now()
    NotificationsTable.update({
