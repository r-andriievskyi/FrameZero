...
class ProductionsRemoteMediatorTest {
  ...
  @Test
  fun `REFRESH writes API page into the cache and stores the next cursor`() = runTest {
  ...
  @Test
  fun `REFRESH with null nextCursor signals end of pagination`() = runTest {
  ...
  @Test
  fun `REFRESH on one filter does not touch rows belonging to other filters`() = runTest {
  ...
  @Test
  fun `APPEND with no stored remote key short-circuits without calling the API`() = runTest {
  ...
  @Test
  fun `APPEND with a stored cursor calls API and continues pageOrder from max + 1`() = runTest {
  ...
  @Test
  fun `PREPEND always short-circuits without calling the API`() = runTest {
    val api = FakeProductionsApi(page = pageOf(nextCursor = null))
  ...
  @Test
  fun `API failure returns Error and leaves the cache untouched`() = runTest {
  ...
  @Test
  fun `CancellationException is rethrown, not converted to Error`() = runTest {
  ...
  private fun productionEntity(id: String, filter: String): ProductionEntity =
    ProductionEntity(
      id = id,
      phaseFilter = filter,
      title = "Title $id",
  ...
  private class FakeProductionsDao : ProductionsDao() {
    ...
    fun updateKeyValue(key: String, value: ProductionRemoteKeyEntity) {
      keys[key] = value
    }
