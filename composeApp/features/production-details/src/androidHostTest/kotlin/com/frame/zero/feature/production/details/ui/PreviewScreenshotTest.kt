...
/**
 * Golden-image coverage for every `@LightDarkPreview` (and any other `@Preview`) in the
 * production-details feature. The scanner discovers previews on the test classpath; the custom
 * `@LightDarkPreview` meta-annotation expands to Light + Dark `@Preview`s, so each composable yields
 * two goldens.
 *
 * Record with `:composeApp:features:production-details:recordRoborazziAndroidHostTest`, check drift with
 * `:composeApp:features:production-details:verifyRoborazziAndroidHostTest`. Robolectric native graphics + SDK
 * level come from `androidHostTest/resources/robolectric.properties`.
 */
...
class PreviewScreenshotTest(
  ...
  companion object {
    ...
    // exact-match by excluding them. (The screen preview renders the loaded state, no spinner.)
    ...
    fun previews(): List<ComposablePreview<AndroidPreviewInfo>> =
      AndroidComposablePreviewScanner()
        .scanPackageTrees("com.frame.zero.feature.production.details.ui")
