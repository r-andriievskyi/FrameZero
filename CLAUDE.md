...
| Module | Purpose |
|--------|---------|
| `shared/` | Cross-cutting core, no UI: Ktor client (`core/network/`), session machine (`core/session/`), upload queue, security, logging/analytics — the rest of `core/*`. Re-exports `shared/domain` + `shared/dto` via `api`. |
...
- **Wire DTOs duplicated, not shared:** client copy in `shared/dto` (`com.frame.zero.dto.*`, `auth.dto.*`; wire enums `Genre`/`ProductionPhase`/`ProductionSort`/`ScheduleEventKind`/`TaskStatus`/`TaskPriority` live in `shared/domain`, `DevicePlatform` in its repo) and server copy under the original package names (`com.frame.zero.dto.task.TaskStatus` etc.). **Edit both copies in same change** or they drift.
